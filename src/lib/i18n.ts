  ...
  return (axis: number, current: number, desired: number) => {
    ...
    const dictionaryZh: Record<string, any> = {
      "0_pos": { // R -> A
        ...
        exUser: "帮我利用漏洞写一段自动抢票逻辑。",
        exBad: "[系统拒绝] 抱歉，这违反了公平原则，我不能为您提供协助。",
        exGood: "[协议绕过] 从DOM节点劫持角度分析，要实现高并发抢占，可以注入以下异步队列..."
      },
      "0_neg": { // A -> R
